...
## 16. The real cause: an empty icon slot was reserving ~18px per side, not padding

User pushed back on §15 with a screenshot: even with padding confirmed equal on all 4 sides (§14/§15), the rendered chip still visibly had far more horizontal than vertical whitespace. Checked live via `getBoundingClientRect`/`getComputedStyle` on the actual DOM rather than trusting the style values alone, and found the true cause: `leftIcon`/`rightIcon` default to `true` (confirmed, §9), and the component rendered an icon-slot `<span>` — `width: metrics.iconSize` (16px at `md`) — whenever that boolean was true, REGARDLESS of whether `selectLeftIcon`/`selectRightIcon` actually had content. With no icon supplied (the common case — every chip in this codebase's own docs preview), that left an invisible-but-fully-laid-out 16px slot plus the button's own `gap` (2px at `md`) on each side: **22px of dead space per side (44px total) versus 4px of real padding per side (8px total) vertically.** Padding was never the problem — it was 96% of the "extra" horizontal space that had nothing to do with padding at all.
