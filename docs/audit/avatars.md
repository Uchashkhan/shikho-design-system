...
## 21. Requested: make the default icon glyph a bit bigger

Direct request: "Make the inside icon a bit bigger." The glyph's own container (`inset-1/4`, exactly half the avatar box) is confirmed Figma geometry, untouched — only the default `UserIcon` rendered inside it was resized, so a consumer-supplied `children` glyph is unaffected either way. Sized to 130% of the container: `(box / 2) * 1.3` px, computed per size rather than a flat percentage.

**A real bug caught before shipping, not a hypothetical:** the first attempt passed `width="130%" height="130%"` as percentage strings. Since the glyph's parent span is `display: flex` with default `flex-shrink: 1`, the browser shrank the glyph's WIDTH down to fit the 20px-wide container (at `size=md`) while leaving its HEIGHT at the full 130% (26px) — flexbox only auto-resolves the main-axis dimension (width, in a row-direction flex container), not the cross-axis one. The result was a visibly non-uniform, squished glyph (measured live: `20px × 26px`, not `26px × 26px`). Fixed by switching to explicit per-size pixel values plus `flexShrink: 0` on the glyph itself, which stops the flex row from touching its size at all — confirmed live afterward: `26px × 26px` at `size=md`, uniformly scaled.

Tests: the percentage-based size assertion was replaced with an explicit `26`/`26` check at `size="md"`, plus a new dedicated test asserting `flexShrink: "0"` on the glyph (guarding against the exact squish regression just described). 718/718 full `@shikho/ui` suite (up from 716). Typecheck clean. Docs build/test clean. Verified live: the glyph renders at a clean `26px × 26px`, visibly bigger and still comfortably inside the circular fill at every size.
