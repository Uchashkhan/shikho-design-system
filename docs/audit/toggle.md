...
- **`switch_ON`**: `base` = `Text/Primary 500` (`#5468ff`). Knob = a `check` layer: a white stadium shape (identical size to `switch_OFF`'s knob) with the same shadow, containing a checkmark icon path filled `#5468FF` (primary/500).
