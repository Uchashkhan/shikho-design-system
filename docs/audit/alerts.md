...
## 16. Follow-up: "Learn more" / close button contrast against the now-tinted surface

Direct follow-up with a screenshot: §15's neutral `gray/100` fill on "Learn more" and the corner close button (both confirmed exact, §11/§14) reads fine against `Default`'s white surface, but loses contrast once §15 tinted the other 4 surfaces — `gray/100` sits too close to e.g. `danger/50`'s own pale pink, making both buttons look washed-out.

Fixed for every severity except `Default` (explicitly confirmed to still work as-is, unchanged): both buttons are now solid white (`white/950`) with a `1px` border using the **exact same border color already applied to the alert's own outer border** (`borderColorByState`) — reusing an already-confirmed value rather than inventing a new border color. `Default` keeps its original `gray/100` fill and no border. Hover target is unchanged (`gray/200`) regardless of the new resting fill.

New tables added to `alert.tsx`: `neutralButtonBgByState`, `neutralButtonBorderByState`. Tests updated throughout `alert.test.tsx` for the new fill/border split between `Default` and the other 4 severities. 711/711 passing (`@shikho/ui`, up from 709). Typecheck clean. Docs build clean. Verified live: `danger`'s "Learn more"/close button now render solid white with a visible pink-red outline; `success` renders the same with a green outline; `Default` is unchanged (gray fill, no border).
